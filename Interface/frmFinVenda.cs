...
namespace Interface
{
    public partial class frmFinVenda : Form
    {
        ...
        private void chkAcrescimo_CheckedChanged(object sender, EventArgs e)
        {
        ...
        private void bntFin_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                {
                    IRetorno = RegraNegocio.Fiscal.Bematech_FI_VendeItem(dadosProdutos["CODIGO_BARRAS", i].Value.ToString(),
                ...
                IRetorno = RegraNegocio.Fiscal.Bematech_FI_IniciaFechamentoCupom(auxAcrescimoDesconto, "%", valorAcrescimoDesconto);
                RegraNegocio.Fiscal.Analisa_iRetorno(IRetorno);

                IRetorno = RegraNegocio.Fiscal.Bematech_FI_EfetuaFormaPagamento(cboPagamento.Text, txtValorPago.Text.Substring(3));
                RegraNegocio.Fiscal.Analisa_iRetorno(IRetorno);

                IRetorno = RegraNegocio.Fiscal.Bematech_FI_TerminaFechamentoCupom("OBRIGADO! VOLTE SEMPRE!");
                RegraNegocio.Fiscal.Analisa_iRetorno(IRetorno);
            ...
            try
            {
                if (MessageBox.Show("Deseja realmente cancelar a venda?", "Deseja excluir?", MessageBoxButtons.YesNo, MessageBoxIcon.Question) == DialogResult.Yes && dadosProdutos.Rows.Count > 0)
                {
                    ...
                    for (int i = 0; i < dadosProdutos.Rows.Count; i++)
                    {
                        ...
                        RegraNegocio.ProdutoRegraNegocio produtos = new RegraNegocio.ProdutoRegraNegocio();
                        ...
                        produtos = new RegraNegocio.ProdutoRegraNegocio();
        ...
        private void frmFinVenda_Load(object sender, EventArgs e)
        {
            ...
            for (int i = 0; i < dadosProdutos.Rows.Count; i++)
            {
                novoItem = i + 1 + "\t" + dadosProdutos["CODIGOBARRAS_PRODUTO", i].Value + "\t" +
        ...
        public frmFinVenda(string subTotal, DataGridView produtos, string documentoCliente,
            int idCliente, int idUsuario, int idVenda, frmVendas frmVendas)
        {
            InitializeComponent();
